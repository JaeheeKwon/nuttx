 ...
 *     chip/stm32h5xxxx_rcc.h {RCC_CFGR_MCO1_HSI, RCC_CFGR_MCO1_LSE,
 *     RCC_CFGR_MCO1_HSE, RCC_CFGR_MCO1_PLL}
 *   div - One of the definitions for the RCC_CFGR_MCO1PRE definitions from
 *     chip/stm32h5xxxx_rcc.h {RCC_CFGR_MCO1PRE_NONE, RCC_CFGR_MCO1PRE_DIV2,
...
static inline void stm32_mco1config(uint32_t source, uint32_t div)
{
  ...
  regval &= ~(RCC_CFGR1_MCO1_MASK | RCC_CFGR1_MCO1PRE_MASK);
  regval |= (source | div);
...
static inline void stm32_mco2config(uint32_t source, uint32_t div)
{
  ...
  regval = getreg32(STM32_RCC_CFGR1);
  regval &= ~(RCC_CFGR1_MCO2_MASK | RCC_CFGR1_MCO2PRE_MASK);
  regval |= (source | div);
  putreg32(regval, STM32_RCC_CFGR1);
}
...
#ifdef CONFIG_STM32H5_CUSTOM_CLOCKCONFIG
