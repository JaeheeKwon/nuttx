/****************************************************************************
 * arch/arm/src/stm32h5/stm32_qspi.c
 *
...
#  undef CONFIG_STM32H5_QSPI_DMADEBUG
#endif
...
#if defined(CONFIG_STM32H5_QSPI_INTERRUPTS) && defined(CONFIG_STM32H5_QSPI_DMA)
...
#  if defined(CONFIG_STM32H5_QSPI_DMAPRIORITY_LOW)
#    define QSPI_DMA_PRIO  DMA_SCR_PRILO
#  elif defined(CONFIG_STM32H5_QSPI_DMAPRIORITY_MEDIUM)
#    define QSPI_DMA_PRIO  DMA_SCR_PRIMED
#  elif defined(CONFIG_STM32H5_QSPI_DMAPRIORITY_HIGH)
#    define QSPI_DMA_PRIO  DMA_SCR_PRIHI
#  elif defined(CONFIG_STM32H5_QSPI_DMAPRIORITY_VERYHIGH)
#    define QSPI_DMA_PRIO  DMA_SCR_PRIVERYHI
#  else
#    define QSPI_DMA_PRIO  DMA_SCR_PRIMED
#  endif

#endif /* CONFIG_STM32H5_QSPI_DMA */

#ifndef STM32_SYSCLK_FREQUENCY
#  error your board.h needs to define the value of STM32_SYSCLK_FREQUENCY
#endif

#if !defined(CONFIG_STM32H5_QSPI_FLASH_SIZE) || 0 == CONFIG_STM32H5_QSPI_FLASH_SIZE
 ...
 *
 * NOTE: the STM32H5 supports only a single QSPI peripheral.  Logic here is
 * designed to support multiple QSPI peripherals.
 */

struct stm32h5_qspidev_s
{
...
#ifdef CONFIG_STM32H5_QSPI_DMADEBUG
  struct stm32h5_dmaregs_s dmaregs[DMA_NSAMPLES];
#endif
...
#ifdef CONFIG_STM32H5_QSPI_REGDEBUG
static bool     qspi_checkreg(struct stm32h5_qspidev_s *priv, bool wr,
                  uint32_t value, uint32_t address);
#else
#  define       qspi_checkreg(priv,wr,value,address) (false)
#endif

static inline uint32_t qspi_getreg(struct stm32h5_qspidev_s *priv,
                  unsigned int offset);
static inline void qspi_putreg(struct stm32h5_qspidev_s *priv,
                  uint32_t value, unsigned int offset);

#ifdef CONFIG_DEBUG_SPI_INFO
static void     qspi_dumpregs(struct stm32h5_qspidev_s *priv,
                  const char *msg);
#else
...
#  ifdef CONFIG_STM32H5_QSPI_DMADEBUG
#    define qspi_dma_sample(s,i) stm32h5_dmasample((s)->dmach, &(s)->dmaregs[i])
static void     qspi_dma_sampleinit(struct stm32h5_qspidev_s *priv);
static void     qspi_dma_sampledone(struct stm32h5_qspidev_s *priv);
#  else
...
#  ifndef CONFIG_STM32H5_QSPI_DMATHRESHOLD
#    define CONFIG_STM32H5_QSPI_DMATHRESHOLD 4
#  endif
...
static int      qspi_hw_initialize(struct stm32h5_qspidev_s *priv);
...
static struct stm32h5_qspidev_s g_qspi0dev =
{
  .qspi              =
  {
    .ops             = &g_qspi0ops,
...
#ifdef CONFIG_DEBUG_SPI_INFO
static void qspi_dumpregs(struct stm32h5_qspidev_s *priv, const char *msg)
{
  spiinfo("%s:\n", msg);
...
static void qspi_dma_sampleinit(struct stm32h5_qspidev_s *priv)
{
  ...
  memset(priv->dmaregs, 0xff,
         DMA_NSAMPLES * sizeof(struct stm32h5_dmaregs_s));

  /* Then get the initial samples */

  stm32h5_dmasample(priv->dmach, &priv->dmaregs[DMA_INITIAL]);
}
...
static void qspi_dma_sampledone(struct stm32h5_qspidev_s *priv)
{
  ...
  stm32h5_dmasample(priv->dmach, &priv->dmaregs[DMA_END_TRANSFER]);

  /* Then dump the sampled DMA registers */

  /* Initial register values */

  stm32h5_dmadump(priv->dmach, &priv->dmaregs[DMA_INITIAL],
              "Initial Registers");

  /* Register values after DMA setup */

  stm32h5_dmadump(priv->dmach, &priv->dmaregs[DMA_AFTER_SETUP],
              "After DMA Setup");

  /* Register values after DMA start */

  stm32h5_dmadump(priv->dmach, &priv->dmaregs[DMA_AFTER_START],
              "After DMA Start");
  ...
  if (priv->result == -ETIMEDOUT)
    {
      stm32h5_dmadump(priv->dmach, &priv->dmaregs[DMA_TIMEOUT],
                  "At DMA timeout");
    }
  else
    {
      stm32h5_dmadump(priv->dmach, &priv->dmaregs[DMA_CALLBACK],
                  "At DMA callback");
    }

  stm32h5_dmadump(priv->dmach, &priv->dmaregs[DMA_END_TRANSFER],
              "At End-of-Transfer");
}
#endif
...
static void qspi_waitstatusflags(struct stm32h5_qspidev_s *priv,
...
static void qspi_abort(struct stm32h5_qspidev_s *priv)
{
  uint32_t regval;
...
static void qspi_ccrconfig(struct stm32h5_qspidev_s *priv,
...
#elif defined(CONFIG_STM32H5_QSPI_DMA)
/****************************************************************************
...
{
  struct stm32h5_qspidev_s *priv = (struct stm32h5_qspidev_s *)arg;
...
static inline uintptr_t qspi_regaddr(struct stm32h5_qspidev_s *priv,
                                    unsigned int offset)
{
...
static int qspi_memory_dma(struct stm32h5_qspidev_s *priv,
                           struct qspi_meminfo_s *meminfo,
                           struct qspi_xctnspec_s *xctn)
{
  uint32_t dmaflags;
...
static int qspi_receive_blocking(struct stm32h5_qspidev_s *priv,
                                 struct qspi_xctnspec_s *xctn)
{
  int ret = OK;
  volatile uint32_t *datareg =
...
static int qspi_transmit_blocking(struct stm32h5_qspidev_s *priv,
                                 struct qspi_xctnspec_s *xctn)
{
  int ret = OK;
  volatile uint32_t *datareg =
...
static int qspi_lock(struct qspi_dev_s *dev, bool lock)
{
  struct stm32h5_qspidev_s *priv = (struct stm32h5_qspidev_s *)dev;
  int ret;
 ...
 *   NOTE:  the STM32H5 QSPI supports only modes 0 and 3.
 *
 * Input Parameters:
 *   dev -  Device-specific state data
 ...
 *   NOTE:  the STM32H5 QSPI only supports 8 bits, so this does nothing.
 *
 * Input Parameters:
 *   dev -  Device-specific state data
...
static void qspi_setbits(struct qspi_dev_s *dev, int nbits)
{
  /* Not meaningful for the STM32H5x6 */
                       ...
                       struct qspi_meminfo_s *meminfo)
{
  ...
  if (priv->candma &&
      meminfo->buflen > CONFIG_STM32H5_QSPI_DMATHRESHOLD &&
...
static int qspi_hw_initialize(struct stm32h5_qspidev_s *priv)
{
...
#if defined(CONFIG_STM32H5_QSPI_MODE_BANK2)
  regval |= QSPI_CR_FSEL;
#endif

#if defined(CONFIG_STM32H5_QSPI_MODE_DUAL)
  regval |= QSPI_CR_DFM;
#endif
  ...
  regval |= ((CONFIG_STM32H5_QSPI_FIFO_THESHOLD - 1) <<
  ...
  regval |= (0x00);
  regval |= ((CONFIG_STM32H5_QSPI_CSHT - 1) << QSPI_DCR_CSHT_SHIFT);
  if (0 != CONFIG_STM32H5_QSPI_FLASH_SIZE)
    {
      unsigned int nsize = CONFIG_STM32H5_QSPI_FLASH_SIZE;
      int nlog2size = 31;
...
struct qspi_dev_s *stm32h5_qspi_initialize(int intf)
{
  struct stm32h5_qspidev_s *priv;
  uint32_t regval;
  int ret;

  /* The STM32H5 has only a single QSPI port */
...
/****************************************************************************
 * Name: stm32h5_qspi_enter_memorymapped
 *
 * Description:
...
void stm32h5_qspi_enter_memorymapped(struct qspi_dev_s *dev,
                                     const struct qspi_meminfo_s *meminfo,
                                     uint32_t lpto)
{
  struct stm32h5_qspidev_s *priv = (struct stm32h5_qspidev_s *)dev;
...
/****************************************************************************
 * Name: stm32h5_qspi_exit_memorymapped
 *
 * Description:
...
void stm32h5_qspi_exit_memorymapped(struct qspi_dev_s *dev)
{
  struct stm32h5_qspidev_s *priv = (struct stm32h5_qspidev_s *)dev;
