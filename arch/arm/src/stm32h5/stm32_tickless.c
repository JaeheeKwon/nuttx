/****************************************************************************
 * arch/arm/src/stm32h5/stm32_tickless.c
 *
...
#if (CONFIG_STM32H5_TICKLESS_TIMER == 2) || \
    (CONFIG_STM32H5_TICKLESS_TIMER == 5)
 #define HAVE_32BIT_TICKLESS 1
#endif

#if (CONFIG_STM32H5_TICKLESS_TIMER == 6) || \
    (CONFIG_STM32H5_TICKLESS_TIMER == 7)
# error Basic timers not supported by the tickless driver
#endif

#if CONFIG_STM32H5_TICKLESS_CHANNEL == 1
#define DIER_CAPT_IE          GTIM_DIER_CC1IE
#elif CONFIG_STM32H5_TICKLESS_CHANNEL == 2
#define DIER_CAPT_IE          GTIM_DIER_CC2IE
#elif CONFIG_STM32H5_TICKLESS_CHANNEL == 3
#define DIER_CAPT_IE          GTIM_DIER_CC3IE
#elif CONFIG_STM32H5_TICKLESS_CHANNEL == 4
...
{
  switch (CONFIG_STM32H5_TICKLESS_TIMER)
    {
  ...
  g_tickless.timer     = CONFIG_STM32H5_TICKLESS_TIMER;
  g_tickless.channel   = CONFIG_STM32H5_TICKLESS_CHANNEL;
  g_tickless.pending   = false;
  g_tickless.period    = 0;
...
#if defined(CONFIG_ARMV8M_SYSTICK) && defined(CONFIG_CPULOAD_PERIOD)
...
int up_alarm_start(const struct timespec *ts)
{
  ...
  STM32_TIM_SETCOMPARE(g_tickless.tch, CONFIG_STM32H5_TICKLESS_CHANNEL, tm);

  stm32_tickless_ackint(g_tickless.channel);
  stm32_tickless_enableint(CONFIG_STM32H5_TICKLESS_CHANNEL);
  ...
  while (tm <= stm32_get_counter())
    {
      ...
      STM32_TIM_SETCOMPARE(g_tickless.tch, CONFIG_STM32H5_TICKLESS_CHANNEL,
                           tm);
    }
...
int up_alarm_cancel(struct timespec *ts)
{
  ...
  stm32_tickless_disableint(CONFIG_STM32H5_TICKLESS_CHANNEL);
