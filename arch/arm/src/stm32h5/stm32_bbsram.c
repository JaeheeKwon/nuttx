/****************************************************************************
 * arch/arm/src/stm32h5/stm32_bbsram.c
 *
...
#ifdef CONFIG_STM32H5_BBSRAM

/****************************************************************************
 * Pre-processor Definitions
 ****************************************************************************/

#if !defined(CONFIG_STM32H5_BKPSRAM)
#error Driver Requires CONFIG_STM32H5_BKPSRAM to be enabled
#endif
...
#if defined(CONFIG_BBSRAM_DEBUG)
static uint8_t debug[STM32H5_BBSRAM_SIZE];
#endif
...
static struct stm32_bbsram_s g_bbsram[CONFIG_STM32H5_BBSRAM_FILES];
                              ...
                              unsigned long arg)
{
  ...
  if (cmd == STM32H5_BBSRAM_GETDESC_IOCTL)
    {
...
static int stm32_bbsram_probe(int *ent, struct stm32_bbsram_s pdev[])
{
  ...
  avail = STM32H5_BBSRAM_SIZE;

  for (i = 0; (i < CONFIG_STM32H5_BBSRAM_FILES) && ent[i] && (avail > 0);
       i++)
    {
...
int stm32_bbsraminitialize(char *devpath, int *sizes)
{
  ...
  /* ST placed the H5's BBSRAM in the default region for SRAM so we need to
   * make it not cacheable
   */

#  if defined(CONFIG_BUILD_PROTECTED)
  mpu_peripheral(STM32_BBSRAM_BASE, STM32H5_BBSRAM_SIZE);
#  else
  mpu_user_peripheral(STM32_BBSRAM_BASE, STM32H5_BBSRAM_SIZE);
...
#  endif
   ...
   * unconditionally in stm32h5xxxx_rcc.c on power up.  This done
...
#if defined(CONFIG_STM32H5_SAVE_CRASHDUMP)
  ...
  if (!once)
    {
      ...
      DEBUGASSERT(fileno > 0 && fileno < CONFIG_STM32H5_BBSRAM_FILES);
