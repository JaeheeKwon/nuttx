/****************************************************************************
 * arch/arm/src/stm32h5/stm32_spi_slave.c
 *
...
#if defined(CONFIG_STM32H5_SPI1_SLAVE) ||                               \
  defined(CONFIG_STM32H5_SPI2_SLAVE) ||                                 \
  defined(CONFIG_STM32H5_SPI3_SLAVE) ||                                 \
  defined(CONFIG_STM32H5_SPI4_SLAVE) ||                                 \
  defined(CONFIG_STM32H5_SPI5_SLAVE) ||                                 \
  defined(CONFIG_STM32H5_SPI6_SLAVE)
...
#if defined(CONFIG_STM32H5_SPI1_SLAVE) || defined(CONFIG_STM32H5_SPI2_SLAVE) || \
  defined(CONFIG_STM32H5_SPI3_SLAVE)
...
#if defined(CONFIG_STM32H5_SPI4_SLAVE) || defined(CONFIG_STM32H5_SPI5_SLAVE)
...
#if defined(CONFIG_STM32H5_SPI6_SLAVE)
...
#if defined (CONFIG_STM32H5_SPI_SLAVE_QSIZE)
#  if CONFIG_STM32H5_SPI_SLAVE_QSIZE > 65535
#    error CONFIG_STM32H5_SPI_SLAVE_QSIZE too large
#  endif
#endif
...
{                                                       \
  ...
  .config        = CONFIG_STM32H5_SPI##x##_COMMTYPE,    \
}

#ifdef CONFIG_STM32H5_SPI1_SLAVE

static
uint8_t SPI_SLAVE_OUTQ(1)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static
uint8_t SPI_SLAVE_INQ(1)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static struct stm32_spidev_s g_spi1ctrlr = SPI_SLAVE_INIT(1);

#endif

#ifdef CONFIG_STM32H5_SPI2_SLAVE

static
uint8_t SPI_SLAVE_OUTQ(2)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static
uint8_t SPI_SLAVE_INQ(2)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static struct stm32_spidev_s g_spi2ctrlr = SPI_SLAVE_INIT(2);

#endif

#ifdef CONFIG_STM32H5_SPI3_SLAVE

static
uint8_t SPI_SLAVE_OUTQ(3)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static
uint8_t SPI_SLAVE_INQ(3)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static struct stm32_spidev_s g_spi3ctrlr = SPI_SLAVE_INIT(3);

#endif

#ifdef CONFIG_STM32H5_SPI4_SLAVE

static
uint8_t SPI_SLAVE_OUTQ(4)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static
uint8_t SPI_SLAVE_INQ(4)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static struct stm32_spidev_s g_spi4ctrlr = SPI_SLAVE_INIT(4);

#endif

#ifdef CONFIG_STM32H5_SPI5_SLAVE

static
uint8_t SPI_SLAVE_OUTQ(5)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static
uint8_t SPI_SLAVE_INQ(5)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static struct stm32_spidev_s g_spi5ctrlr = SPI_SLAVE_INIT(5);

#endif

#ifdef CONFIG_STM32H5_SPI6_SLAVE

/* TODO: these needs to be located in SRAM3 for SPI6 */

static
uint8_t SPI_SLAVE_OUTQ(6)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
static
uint8_t SPI_SLAVE_INQ(6)[DMA_ALIGN_UP(CONFIG_STM32H5_SPI_SLAVE_QSIZE)]
aligned_data(ARMV8M_DCACHE_LINESIZE);
...
static void spi_dmarxsetup(struct stm32_spidev_s *priv, size_t nwords)
{
  ...
  dmacfg.ndata = CONFIG_STM32H5_SPI_SLAVE_QSIZE;
                     ...
                     int nbits)
{
  ...
  up_flush_dcache((uintptr_t)priv->inq,
                  (uintptr_t)priv->inq + CONFIG_STM32H5_SPI_SLAVE_QSIZE);
...
#ifdef CONFIG_STM32H5_SPI_DMA
  priv->ihead =
    CONFIG_STM32H5_SPI_SLAVE_QSIZE - stm32_dmaresidual(priv->rxdma);
#else
  priv->ihead = 0;
  priv->itail = 0;
#endif
...
static inline int spi_rx_buffer_free(uint8_t *ptr, int start, int end)
{
  ...
  if (end >= CONFIG_STM32H5_SPI_SLAVE_QSIZE)
    {
      end = 0;
      up_invalidate_dcache((uintptr_t)&ptr[start],
                           (uintptr_t)&ptr[CONFIG_STM32H5_SPI_SLAVE_QSIZE]);
    }
  else
    {
      up_invalidate_dcache((uintptr_t)&ptr[start],
...
static size_t spi_qpoll(struct spi_slave_ctrlr_s *ctrlr)
{
  ...
  DEBUGASSERT(priv != NULL && priv->dev != NULL);
  DEBUGASSERT(priv->ihead < CONFIG_STM32H5_SPI_SLAVE_QSIZE);
...
#ifdef CONFIG_STM32H5_SPI_DMA
  itail = CONFIG_STM32H5_SPI_SLAVE_QSIZE - stm32_dmaresidual(priv->rxdma);
#else
  ...
  if (ihead > itail)
    {
      ...
      priv->ihead += SPIS_DEV_RECEIVE(priv->dev,
                                      ...
                                      CONFIG_STM32H5_SPI_SLAVE_QSIZE -
                                        ihead);
  ...
  bytes_left = itail < priv->ihead
    ? CONFIG_STM32H5_SPI_SLAVE_QSIZE - priv->ihead + itail
    : itail - priv->ihead;
...
#endif /* CONFIG_STM32H5_SPI1..6_SLAVE */
