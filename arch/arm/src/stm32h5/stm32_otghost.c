/****************************************************************************
 * arch/arm/src/stm32h5/stm32_otghost.c
 *
...
#if defined(CONFIG_USBHOST) && defined(CONFIG_STM32H5_OTGFS)
...
#if defined(CONFIG_STM32H5_OTGFS_HOST)
...
#elif defined(CONFIG_STM32H5_OTGHS_HOST)
...
#if defined(CONFIG_STM32H5_OTGFS_HOST) && defined(CONFIG_STM32H5_OTGHS_HOST)
 ...
 *  CONFIG_STM32H5_OTGFS  - Enable the STM32 USB OTG FS block
 *  CONFIG_STM32H5_SYSCFG - Needed
 *
 * Options:
 *
 *  CONFIG_STM32H5_OTG_RXFIFO_SIZE - Size of the RX FIFO in 32-bit words.
 *    Default 128 (512 bytes)
 *  CONFIG_STM32H5_OTG_NPTXFIFO_SIZE - Size of the non-periodic Tx FIFO
 *    in 32-bit words.  Default 96 (384 bytes)
 *  CONFIG_STM32H5_OTG_PTXFIFO_SIZE - Size of the periodic Tx FIFO in 32-bit
 *    words.  Default 96 (384 bytes)
 *  CONFIG_STM32H5_OTG_DESCSIZE - Maximum size of a descriptor.  Default: 128
 *  CONFIG_STM32H5_OTG_SOFINTR - Enable SOF interrupts.  Why would you ever
 *    want to do that?
 *  CONFIG_STM32H5_USBHOST_REGDEBUG - Enable very low-level register access
 *    debug.  Depends on CONFIG_DEBUG_FEATURES.
 *  CONFIG_STM32H5_USBHOST_PKTDUMP - Dump all incoming and outgoing USB
 *    packets. Depends on CONFIG_DEBUG_FEATURES.
 */

/* Pre-requisites (partial) */

#ifndef CONFIG_STM32H5_SYSCFG
#  error "CONFIG_STM32H5_SYSCFG is required"
#endif

/* Default RxFIFO size */

#ifndef CONFIG_STM32H5_OTG_RXFIFO_SIZE
#  define CONFIG_STM32H5_OTG_RXFIFO_SIZE 128
#endif

/* Default host non-periodic Tx FIFO size */

#ifndef CONFIG_STM32H5_OTG_NPTXFIFO_SIZE
#  define CONFIG_STM32H5_OTG_NPTXFIFO_SIZE 96
#endif

/* Default host periodic Tx fifo size register */

#ifndef CONFIG_STM32H5_OTG_PTXFIFO_SIZE
#  define CONFIG_STM32H5_OTG_PTXFIFO_SIZE 96
#endif

/* Maximum size of a descriptor */

#ifndef CONFIG_STM32H5_OTG_DESCSIZE
#  define CONFIG_STM32H5_OTG_DESCSIZE 128
#endif

/* Register/packet debug depends on CONFIG_DEBUG_FEATURES */

#ifndef CONFIG_DEBUG_FEATURES
#  undef CONFIG_STM32H5_USBHOST_REGDEBUG
#  undef CONFIG_STM32H5_USBHOST_PKTDUMP
#endif
...
#ifdef CONFIG_STM32H5_USBHOST_REGDEBUG
...
#ifdef CONFIG_STM32H5_USBHOST_PKTDUMP
...
#ifdef CONFIG_STM32H5_OTG_SOFINTR
                       ...
                       uint8_t **buffer, size_t *maxlen)
{
  ...
  alloc = kmm_malloc(CONFIG_STM32H5_OTG_DESCSIZE);
  if (!alloc)
    {
      return -ENOMEM;
    }
  ...
  *buffer = alloc;
  *maxlen = CONFIG_STM32H5_OTG_DESCSIZE;
  return OK;
}
...
static void stm32_host_initialize(struct stm32_usbhost_s *priv)
{
  ...
  stm32_putreg(STM32_OTG_GRXFSIZ, CONFIG_STM32H5_OTG_RXFIFO_SIZE);
  offset = CONFIG_STM32H5_OTG_RXFIFO_SIZE;

  /* Setup the host non-periodic Tx FIFO size (HNPTXFSIZ) */

  regval = (offset |
            (CONFIG_STM32H5_OTG_NPTXFIFO_SIZE <<
            OTG_HNPTXFSIZ_NPTXFD_SHIFT));
  stm32_putreg(STM32_OTG_HNPTXFSIZ, regval);
  offset += CONFIG_STM32H5_OTG_NPTXFIFO_SIZE;

  /* Set up the host periodic Tx fifo size register (HPTXFSIZ) */

  regval = (offset |
            (CONFIG_STM32H5_OTG_PTXFIFO_SIZE <<
...
#ifdef CONFIG_STM32H5_OTG_USBREGEN
...
#ifdef CONFIG_STM32H5_OTG_SOFOUTPUT
...
#endif /* CONFIG_USBHOST && CONFIG_STM32H5_OTGFS */
